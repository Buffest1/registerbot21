...
client.on("guildMemberAdd", async member => {
    ...
    if (ktarih < 604800000) {
        ...
        if (regKanal) regKanal.send(`<@802489919254822933>`, eresbosemb.setDescription(`\`•\` **Merhaba**, Sunucumuza Hoş Geldin ${member.user} \n\n\`•\` Seninle Birlikte **${member.guild.memberCount}** Kişiyiz. \n\`•\` Hesabın Hesabın `  + moment(member.user.createdAt).format("DD MMMM YYYY dddd") + ` Tarihinde Oluşturulmuş. \n\`•\` Kayıt Olmak İçin **Voice Confirmed** Ses Odalarına Girmelisiniz. \n\n\`•\` **Güvenli Hesap!** ✅ `,true).setColor(0x20cc00));
    }
});
